...
async function fetchAndStoreTracks() {
  ...
  for (const mood of MOODS) {
    ...
    try {
      ...
      console.log(
        `  ✅ ${mood}: +${jamendoTracks.length} tracks (Total: ${totalTracks})`,
      );
  ...
  if (totalTracks < TARGET_TRACKS) {
    ...
    for (const genre of GENRES) {
      ...
      try {
        ...
        console.log(
          `  ✅ ${genre}: +${jamendoTracks.length} tracks (Total: ${totalTracks})`,
        );
        await new Promise((resolve) => setTimeout(resolve, 1000));
      } catch (error) {
  ...
  const csvHeader =
    "id,title,artist,url,duration,genre,mood,license,cover_art,audio_url,has_artwork\n";
  ...
  const artworkPercentage = (
    (tracksWithArtwork / allTracks.length) *
    100
  ).toFixed(1);
