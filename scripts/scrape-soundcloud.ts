...
const GENRES = ['rock', 'pop', 'hip-hop', 'electronic', 'classical', 'jazz', 'ambient', 'lofi', 'metal'];
const MOODS = ['happy', 'sad', 'chill', 'energetic', 'focus', 'relaxed', 'workout'];
 ...
 */
function classify(tags: string[], primaryGenre: string): { genre: string[]; mood: string[] } {
 ...
 */
async function scrapeGenre(genre: string, pageNum: number = 1): Promise<number> {
  ...
  try {
    const searchQuery = encodeURIComponent(`royalty free ${genre} creativecommons`);
    ...
    console.log(`[Scraper] Inserted ${inserted} tracks for ${genre} (page ${pageNum})`);
    return inserted;
  } catch (error) {
