...
const GENRES = [
  'rock', 'pop', 'hip-hop', 'rap', 'r&b', 'soul', 'funk', 'disco',
  'electronic', 'edm', 'techno', 'house', 'dubstep', 'trance', 'drum and bass', 'ambient',
  'lofi', 'chillwave', 'chillhop', 'jazz', 'blues', 'swing', 'classical', 'orchestral', 'piano',
  'acoustic', 'folk', 'indie', 'alternative', 'punk', 'grunge', 'metal', 'heavy metal', 'thrash',
  'reggae', 'dancehall', 'ska', 'latin', 'salsa', 'reggaeton', 'bachata', 'country', 'bluegrass',
  'world', 'african', 'indian', 'k-pop', 'bollywood', 'soundtrack', 'cinematic', 'game',
  'chiptune', 'vaporwave', 'synthwave', 'phonk', 'hyperpop', 'drill', 'trap', 'grime', 'uk drill'
];

// Comprehensive mood list (35+)
const MOODS = [
  'happy', 'joyful', 'upbeat', 'energetic', 'dance', 'party', 'workout', 'running', 'excited',
  'positive', 'uplifting', 'motivational', 'chill', 'relaxed', 'calm', 'peaceful', 'serene',
  'meditative', 'focus', 'study', 'sleep', 'lofi', 'nostalgic', 'melancholic', 'sad', 'emotional',
  'heartbreak', 'reflective', 'dreamy', 'atmospheric', 'epic', 'intense', 'aggressive', 'dark', 'mysterious'
];

// Style tags (25+)
const STYLES = [
  'instrumental', 'vocal', 'female vocal', 'male vocal', 'choir', 'remix', 'cover', 'live',
  'acoustic version', 'piano version', 'guitar', 'synth', 'beats', 'loop', 'sample pack',
  'original', 'demo', 'unplugged', 'stripped', 'orchestral', 'symphonic', 'minimal', 'ambient', 'experimental'
];
...
function classifyTrack(
  ...
  const allText = `${title} ${artist} ${tags.join(' ')} ${searchQuery}`.toLowerCase();
 ...
 */
async function scrapeQuery(query: string, pageNum: number = 1): Promise<number> {
  ...
  try {
    ...
    try {
      await page.waitForSelector('article[data-testid="track"]', { timeout: 10000 });
    } catch (e) {
    ...
    const tracks = await page.evaluate(() => {
      const items = Array.from(document.querySelectorAll('article[data-testid="track"]'));
      return items.slice(0, 20).map((el) => {
        const titleEl = el.querySelector('[data-testid="trackTitle"]') as HTMLElement;
        const artistEl = el.querySelector('[data-testid="trackArtist"]') as HTMLElement;
        const linkEl = el.querySelector('a[href*="/"]') as HTMLAnchorElement;
        const waveformEl = el.querySelector('[data-testid="waveform"]') as HTMLImageElement;
        const durationEl = el.querySelector('[data-testid="trackDuration"]') as HTMLElement;
    ...
    for (const track of tracks) {
      ...
      const currentCount = (db.prepare('SELECT COUNT(*) as count FROM tracks').get() as { count: number }).count;
      if (currentCount >= MAX_TRACKS) {
        console.log(`[Scraper] Reached maximum tracks (${MAX_TRACKS}), stopping...`);
        break;
      }

      // Classify track
      const { genres, moods, styles } = classifyTrack(track.title, track.artist, track.tags, query);
    ...
    console.log(`[Scraper] Added: ${added} | Total: ${totalCount}/${TARGET_TRACKS}`);
...
async function main() {
  ...
  for (const query of queries) {
    ...
    queryIndex++;
    console.log(`[Scraper] Progress: ${queryIndex}/${queries.length} queries processed\n`);
  }
  ...
  const csvHeader = 'id,title,artist,url,duration,genre,mood,style,license,scraped_at\n';
