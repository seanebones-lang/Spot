...
=======
  test('should upload track, redirect to new releases, and display with genre/mood/explicit sticker', async ({ page }) => {
    // Navigate to upload page
    await page.goto('/upload');
...
=======
    ...
    await page.fill('input[placeholder*="track title" i], input[placeholder*="Enter exact track title"]', 'E2E Test Track');
    await page.fill('input[placeholder*="First Name Last Name" i]', 'Test Artist');
...
=======
    await expect(page.locator('text=published successfully')).toBeVisible({ timeout: 10000 });

    // Wait for redirect to new-releases (not dashboard)
    await page.waitForURL('/new-releases', { timeout: 5000 });

    // Verify track appears in new releases
    await expect(page.locator('a:has-text("E2E Test Track")')).toBeVisible({ timeout: 3000 });
...
=======
  test('should persist tracks in localStorage across page refreshes', async ({ page }) => {
    // First, set a track in localStorage
    await page.goto('/dashboard/artist');
    await page.evaluate(() => {
      localStorage.setItem('artist-tracks', JSON.stringify([
        { id: 'test-1', name: 'Persisted Track', album: 'Test Album', uploadDate: new Date().toISOString(), status: 'published', streams: 100, earnings: 0.50 }
      ]));
>>>>>>> 460cde8a4456665eaca40b34f2a2a146c789ce1e
    });
...
=======
    await expect(page.locator('table tbody tr:has-text("Persisted Track")')).toBeVisible();
>>>>>>> 460cde8a4456665eaca40b34f2a2a146c789ce1e
  });
});
