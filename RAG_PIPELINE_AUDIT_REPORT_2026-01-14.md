...
class EmbeddingCache {
  ...
  async getOrCompute(fileHash: string, computeFn: () => Promise<number[]>): Promise<number[]> {
...
function validateEmbedding(embedding: number[], expectedDim: number): void {
  if (embedding.length !== expectedDim) {
    throw new Error(`Embedding dimension mismatch: ${embedding.length} vs ${expectedDim}`);
  }
}
```
