...
export default function MoodPage() {
  ...
  const { setCurrentTrack, setIsPlaying, currentTrack, isPlaying, addToQueue } =
    usePlayerStore();
  ...
  const filteredPlaylists = playlists
    .filter((playlist) => {
      ...
      if (selectedFeelings.length > 0 && playlist.moodTags.feelings) {
        const overlap = selectedFeelings.filter((f) =>
          playlist.moodTags?.feelings.includes(f),
        ).length;
        const overlapScore =
          overlap /
          Math.max(selectedFeelings.length, playlist.moodTags.feelings.length);
      ...
      if (selectedGenres.length > 0 && playlist.moodTags.genres) {
        const genreOverlap = selectedGenres.filter((g) =>
          playlist.moodTags?.genres.includes(g),
        ).length;
        const genreScore =
          genreOverlap /
          Math.max(selectedGenres.length, playlist.moodTags.genres.length);
  ...
  return (
    ...
    >
      ...
      >
        Select your mood, feelings, vibe, and genres to find the perfect
        playlists
      </p>
      ...
      <div className="mt-8" style={{ marginTop: "32px" }}>
        ...
        >
          ...
          >
            ...
            {filteredPlaylists.length !== 1 ? "s" : ""} Match Your Mood
          </h2>
        </div>
        <div
        ...
        >
          {filteredPlaylists.map((playlist) => {
            ...
            const isPlaylistPlaying =
              currentTrack?.id === firstTrackId && isPlaying;
            return (
              <div
                key={playlist.id}
