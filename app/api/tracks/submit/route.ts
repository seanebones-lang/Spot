...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      logger.warn('CSRF token validation failed for track submission', { correlationId });
      return NextResponse.json(
    ...
    try {
      ...
      if (uploadError instanceof Error && uploadError.message.includes('Cloud storage not configured')) {
        logger.error('Cloud storage not configured for file upload', uploadError, { correlationId });
        return NextResponse.json(
          { 
            error: 'File storage is not configured. Please configure S3 or R2 storage for file uploads.',
