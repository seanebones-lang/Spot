...
const PROVIDERS = {
  ...
  aws: {
    ...
    apply: 'aws cloudformation deploy --template-file cloudformation.yaml --stack-name spot-app',
    status: 'aws cloudformation describe-stacks --stack-name spot-app',
    deploy: 'aws cloudformation deploy --template-file cloudformation.yaml --stack-name spot-app',
  },
  gcp: {
    plan: 'gcloud deployment-manager deployments describe spot-app',
    apply: 'gcloud deployment-manager deployments create spot-app --config config.yaml',
    status: 'gcloud deployment-manager deployments describe spot-app',
    deploy: 'gcloud deployment-manager deployments update spot-app --config config.yaml',
  },
  azure: {
    plan: 'az deployment group validate --resource-group spot-app --template-file azuredeploy.json',
    apply: 'az deployment group create --resource-group spot-app --template-file azuredeploy.json',
    status: 'az deployment group show --resource-group spot-app --name spot-app',
    deploy: 'az deployment group create --resource-group spot-app --template-file azuredeploy.json',
  },
};
